    ...
    return (
        <div ref={ref} className="card backdrop-blur-sm relative flex flex-col bg-cyan-500/30 border-4 border-gray-800 rounded-lg p-5 pb-3 min-w-[375px] container shadow-black/40 shadow-xl overflow-hidden" {...props}>
            ...
            <div className="imgDiv relative rounded-lg overflow-hidden border-0">
                <img className='border-0 object-cover w-full h-48' src={details.img} />
                ...
                <div className='link text-white absolute z-[92] left-1/2 transform -translate-x-1/2 top-14 opacity-0'>
                    <a href={details.link} target='_blank' className='flex flex-row flex-nowrap rounded-sm bg-lime-600 py-1 px-9'>
            ...
            <div className="desc px-2 z-[91] absolute bottom-0 opacity-0 text-md font-normal">
                {details.desc.map((line, index) => <div className='text-nowrap max-w-2/3 overflow-hidden text-ellipsis' key={index}>- {line}</div>)}
            </div>


            {/* tech */}
            <div className='z-[91] pl-2 text-sm text-gray-800 flex flex-row gap-2'>
                {details.tech.map((tec, index) => <div key={index}>{`${tec}`}</div>)}
            </div>
        </div>
    )
})
